...
public final class RatTerm {
  ...
  public RatTerm negate() {
    ...
    return new RatTerm(this.getCoeff().negate(), this.getExpt());
  }
  ...
  public RatTerm add(RatTerm arg) {
    checkRep();
    if(this.expt != arg.expt && this.isNaN() == false && arg.isNaN() == false && this.isZero() == false && arg.isZero() == false){
      throw new IllegalArgumentException();
    }

    if(this.isNaN() == true || arg.isNaN() == true ){
      return NaN;
    }

    if(this.isZero()){
      return new RatTerm(this.getCoeff().add(arg.getCoeff()), arg.expt);
    }

    if(arg.isZero()){
      return new RatTerm(this.getCoeff().add(arg.getCoeff()), this.expt);
    }

    return new RatTerm(this.getCoeff().add(arg.coeff), this.getExpt());
  }
  ...
  public RatTerm sub(RatTerm arg) {
    ...
    if(this.expt != arg.expt && !(this.isZero() || arg.isZero() || !this.isNaN() || arg.isNaN() == true)){
      throw new IllegalArgumentException();
    }
    ...
    return new RatTerm(this.getCoeff().sub(arg.coeff), this.getExpt());
  }
  ...
  public RatTerm mul(RatTerm arg) {
    ...
    int exptNew = this.expt + arg.expt;

    return new RatTerm(this.getCoeff().mul(arg.coeff), exptNew);
  }
  ...
  public RatTerm div(RatTerm arg) {
    ...
    int exptNew = this.expt - arg.expt;
    return new RatTerm (this.getCoeff().div(arg.getCoeff()), exptNew);
  }
  ...
  public RatTerm differentiate() {
    RatTerm q = new RatTerm(RatNum.NaN, 0);
    if(this.isNaN()){
        return q;
    }
    if(this.expt == 0){
    ...
    RatNum exptRat = new RatNum(this.expt, 1);

    return new RatTerm (this.getCoeff().mul(exptRat), this.expt - 1);
  }
  ...
  public RatTerm antiDifferentiate() {
    ...
    RatNum exptRat = new RatNum(this.expt + 1, 1);

    return new RatTerm(this.getCoeff().div(exptRat), this.expt+1);
  }
