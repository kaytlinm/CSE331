...
public final class RatPoly {
  ...
  public RatPoly div(RatPoly p) {
    if(p.terms.isEmpty() || this.isNaN() || p.isNaN()){
      ...
      RatPoly r = ZERO;
      RatPoly v = new RatPoly(this.terms);
      RatTerm p0 = p.terms.get(0);
      while (!v.equals(ZERO) && p.degree() <= v.degree()) {
        RatTerm rt = v.terms.get(0).div(p0);
        RatPoly temp = ZERO;

        for(RatTerm tp : p.terms){
          temp = temp.add(new RatPoly(tp.mul(rt)));
        }
        v = v.sub(temp);
        r = r.add(new RatPoly(rt));
  ...
  public RatPoly differentiate() {
    ...
    for(int i = 0; i < this.terms.size(); i++){
      sortedInsert(r, terms.get(i).differentiate());
    }
    return new RatPoly(r);
  ...
  public RatPoly antiDifferentiate(RatNum integrationConstant) {
    if(this.isNaN() || integrationConstant.isNaN()){
      ...
      List<RatTerm> r = new ArrayList<RatTerm>();
      for(int i = 0; i < terms.size(); i++){
        sortedInsert(r, terms.get(i).antiDifferentiate());
      }
      sortedInsert(r,new RatTerm(integrationConstant, 0));
      return new RatPoly(r);
    }
  }
  ...
  public double integrate(double lowerBound, double upperBound) {
    if(this.isNaN() || Double.isNaN(lowerBound) || Double.isNaN(upperBound)){
      ...
      RatPoly r = this.antiDifferentiate(RatNum.ZERO);
