...
/**
 * This represents an immutable path between two Type T start and end, particularly
...
public class Path<T> implements Iterable<Path<T>.Segment> {
  ...
  public Path<T> extend(T newEnd, double segmentCost) {
    ...
    extendedPath.path.add(new Path<T>.Segment(this.getEnd(), newEnd, segmentCost));
  ...
  public T getEnd() {
    ...
    Path<T>.Segment seg = path.get(path.size() - 1);
    return seg.getEnd();
  }
  ...
  public Iterator<Path<T>.Segment> iterator() {
    ...
    return new Iterator<Path<T>.Segment>() {
  ...
  /**
   * Segment represents a single segment as part of a longer, more complex path between types.
  ...
  public class Segment {
     ...
     *
     * @param start The starting of this segment.
     * @param end   The ending of this segment.
    ...
    private Segment(T start, T end, double cost) {
      if(start == null || end == null) {
        throw new NullPointerException("Segments cannot have null start or ends.");
      }
