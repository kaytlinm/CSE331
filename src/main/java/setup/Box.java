...
public class Box implements Iterable<Ball> {
    ...
    public Iterator<Ball> getBallsFromSmallest() {
        ArrayList<Ball> sortedBox = new ArrayList<>();
        ArrayList<Double> boxVolumes = new ArrayList<>();
        Map<Double, Ball> matches = new HashMap<>();

        for(Ball b : ballContainer){
            matches.put(b.getVolume(), b);
            boxVolumes.add(b.getVolume());
        }

        Collections.sort(boxVolumes);
        for(int i = 0; i < boxVolumes.size(); i++){
            sortedBox.add(matches.get(boxVolumes.get(i)));
        }
